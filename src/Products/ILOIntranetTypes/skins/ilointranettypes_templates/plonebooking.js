...
function centerContent(content) {
    var self = jQuery(content);
    self.css("position","absolute");
    self.css("top", ((jQuery(window).height() - self.outerHeight()) / 2) + jQuery(window).scrollTop() + "px");
    self.css("left", ((jQuery(window).width() - self.outerWidth()) / 2) + jQuery(window).scrollLeft() + "px");
}
...
Booking.submit = function (e) {
    ...
    for (i = 0;i < inputs.length;++i)
        if (inputs[i].type == "text" || inputs[i].type == 'checkbox' || (inputs[i].type == "hidden" && inputs[i].className != "doNotSubmit")) {
