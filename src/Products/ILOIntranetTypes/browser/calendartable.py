...
import calendar
from Products.AdvancedQuery import And, Eq, Or, Between
...
class CalendarTableBaseView(grok.View):
    grok.baseclass()
...
class CalendarTableView(CalendarTableBaseView):
    grok.name('calendartable')
    grok.template('calendartable')

class CalendarTablePrintView(CalendarTableBaseView):
    grok.name('calendartable_print')


    def render(self):
        html = self.context.calendartable_print(view=self)
        out = StringIO()
